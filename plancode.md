...
> Quy ước: ✅ = đã có trong scaffold · ⬜ = cần code.
...
- ⬜ **Chưa có entity nào, chưa có module nghiệp vụ, chưa có route login**
...
- ⬜ **Chưa có trang/component nghiệp vụ thật**
...
### GÓI 1 — Auth login (admins)  ⬅️ BẮT ĐẦU TỪ ĐÂY
...
### GÓI 4 — Attributes + Tags (phụ trợ cho sản phẩm)
...
### GÓI 7 — News / Bảng giá (dùng chung bảng `news`)
...
### GÓI 8 — Pages tĩnh + Contacts + Support + Banners + Videos
Các module nhỏ, làm gộp:
- ⬜ `pages/` — `GET /api/public/pages/:slug`, admin CRUD (Giới thiệu, Chính sách…).
- ⬜ `contacts/` — `POST /api/public/contacts` (`@Public`, **rate-limit chặt + honeypot**, lưu IP), admin list/update status.
- ⬜ `support/` — `GET /api/public/support` (hỗ trợ trực tuyến), admin CRUD.
- ⬜ `banners/` — `GET /api/public/banners?position=home_slider`, admin CRUD.
- ⬜ `videos/` — `GET /api/public/videos?position=home` (xử lý cột SET), admin CRUD.
- **Xong khi:** trang chủ & liên hệ có đủ dữ liệu phụ.

### GÓI 9 — SEO kỹ thuật backend
...
### FE-1 — Layout chung (cần BE GÓI 2,3)
...
- ⬜ `app/san-pham/[slug]/page.tsx` — Gallery, thông số, tabs (Mô tả/Hình ảnh/Kết quả thử nghiệm), related, nút "Yêu cầu báo giá". `generateStaticParams` + ISR. **JSON-LD `Product`** (quan trọng nhất). Gọi `POST view`.

### FE-4 — Tin tức & Bảng giá (cần BE 7)
- ⬜ `app/tin-tuc/`, `app/tin-tuc/[slug]/`, `app/bang-gia/`, `app/bang-gia/[slug]/`. JSON-LD `Article`/`ItemList`.

### FE-5 — Nội dung & liên hệ (cần BE 8)
- ⬜ `app/gioi-thieu/`, `app/[slug]/` (trang tĩnh), `app/lien-he/` (form → `contacts`, honeypot/captcha, map). JSON-LD `ContactPage`/`Organization`.

### FE-6 — Tìm kiếm & tag (cần BE 5)
- ⬜ `app/tim-kiem/` (SSR, `noindex`), `app/tag/[slug]/`.

### FE-7 — SEO kỹ thuật (cần BE 9)
- ⬜ Nối `app/sitemap.ts` & `app/robots.ts` vào API thật; `middleware.ts` tra `redirects` (301); kiểm `og:image`, canonical, Rich Results.

---

## 4. ADMIN CMS (sau khi public site chạy)
- ⬜ Khu `/admin` (CSR, `noindex`): login → JWT lưu cookie/localStorage; client gọi `/api/admin/*` kèm Bearer.
- ⬜ Màn hình: Dashboard, Categories, Products (upload nhiều ảnh + attr + tag + tab thử nghiệm), Attributes, Tags, News/Bảng giá (WYSIWYG), Pages, Banners, Videos, Contacts, Support, Settings, Redirects.
- ⬜ Editor WYSIWYG (TinyMCE/CKEditor) chèn ảnh qua `/api/admin/upload`; **sanitize lại server-side**.
